...
export class Backend implements backendInterface {
    ...
    async getAllSubmissions(): Promise<Array<QuotationRequest>> {
        if (this.processError) {
            try {
    ...
    async submitQuotation(arg0: QuotationRequest): Promise<void> {
        if (this.processError) {
            try {
                const result = await this.actor.submitQuotation(arg0);
                return result;
            } catch (e) {
                this.processError(e);
                throw new Error("unreachable");
